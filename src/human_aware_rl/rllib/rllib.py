...
class RlLibAgent(Agent):
    ...
    def action(self, state):
        ...
        action_probabilities_forced = np.zeros((1,6))
        action_prob = 0.8
        action_prob_complement = ((1.0-0.8)/5)
        for i in range(6):
            if i == np.argmax(action_probabilities):
                action_probabilities_forced[0,i] = action_prob
            else:
                action_probabilities_forced[0,i] = action_prob_complement
        ...
        # action_idx = np.argmax(action_probabilities[0])
...
def load_trainer(save_path, true_num_workers=False):
    ...
    config["training_params"]["seed"] = np.random.randint(0,1e6)
...
def get_policy_from_trainer(trainer, policy_id="ppo", agent_index=0):
    policy = trainer.get_policy(policy_id)
    # dummy_env = trainer.env_creator(trainer.config["env_config"])
    # featurize_fn = dummy_env.featurize_fn_map[policy_id]
    # agent = RlLibAgent(policy, agent_index, featurize_fn=featurize_fn)
...
def load_policy(save_path, policy_id="ppo", agent_index=0):
    """
    Returns an RllibAgent (compatible with the Overcooked Agent API) from the `save_path` to a previously
    serialized trainer object created with `save_trainer`
    ...
    """
    trainer = load_trainer(save_path)
    return get_policy_from_trainer(
        trainer, policy_id=policy_id, agent_index=agent_index
    )